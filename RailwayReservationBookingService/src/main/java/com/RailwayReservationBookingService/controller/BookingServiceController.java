...
public class BookingServiceController {
	...
	public String addPassengers(@Valid @RequestBody Passengers passengerDetails) {
		return bookingServiceImpl.addPassengersDetails(passengerDetails);
	...
	public ResponseEntity<?> addUserDetails(@Valid @RequestBody UserDetails userDetails) {
		...
		List<Passengers> pass = userDetails.getPassengers().stream().map(ans1 -> {
		...
		restTemplate.getForObject("http://localhost:8089/admin/updateSeats/" + trainNo + "/" + totalpassenger,
				TrainDetails.class);
